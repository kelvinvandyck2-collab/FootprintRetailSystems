...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Recent Transactions
        let recentWhere = storeLocation ? "WHERE t.store_location = $1" : "";
...
app.post('/products', authenticateToken, async (req, res) => {
    const { barcode, name, category, price, stock, cost_price, selling_unit, packaging_unit, conversion_rate, reorder_level, track_batch, track_expiry } = req.body;
    try {
...
app.put('/products/:barcode', authenticateToken, async (req, res) => {
    const { name, category, price, stock, cost_price, selling_unit, packaging_unit, conversion_rate, reorder_level, track_batch, track_expiry } = req.body;
...
app.get('/transactions/all', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const params = [];
        if (storeLocation) {
            query += ` WHERE t.store_location = $1`;
...
app.get('/transactions/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Fetch system settings for VAT rate
        // Use the store_id from the transaction creator (cashier), fallback to current user's store, then default to 1
        ...
        // Calculate derived values using dynamic VAT rate
...
app.post('/api/transactions', authenticateToken, async (req, res) => {
    ...
    const { items, total, paymentMethod, promoCode, discount, customerId } = req.body;
    ...
    try {
        ...
        try {
            ...
            const txnRes = await client.query(
                ...
                (user_id, store_location, total_amount, payment_method, receipt_number, items, created_at, customer_id, customer_name, status)
                VALUES ($1, $2, $3, $4, $5, $6, NOW(), $7, $8, 'completed') RETURNING id`,
                [req.user.id, req.user.store_location, total, paymentMethod, receiptNumber, JSON.stringify(items), parsedCustomerId, customerName]
            );
